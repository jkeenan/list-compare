...
my $lcma   = List::Compare->new('-a', \@a0, \@a1, \@a2, \@a3, \@a4);
...
eval { $unique_ref = $lcma->get_unique_ref(9) };
...
eval { $complement_ref = $lcma->get_complement_ref(9) };
...
eval { $LR = $lcma->is_LsubsetR(2) };
ok(ok_capture_error($@));               # 614

eval { $LR = $lcma->is_LsubsetR(8,9) };
...
eval { $eqv = $lcma->is_LequivalentR(2) };
ok(ok_capture_error($@));               # 621

eval { $eqv = $lcma->is_LequivalentR(8,9) };
...
eval { $memb_arr_ref = $lcma->is_member_which_ref('jerky', 'zebra') };
...
eval { $memb_hash_ref = $lcma->are_members_which() };
ok(ok_capture_error($@));               # 659


ok($lcma->is_member_any('abel'));       # 660
ok($lcma->is_member_any('baker'));      # 661
ok($lcma->is_member_any('camera'));     # 662
ok($lcma->is_member_any('delta'));      # 663
ok($lcma->is_member_any('edward'));     # 664
ok($lcma->is_member_any('fargo'));      # 665
ok($lcma->is_member_any('golfer'));     # 666
ok($lcma->is_member_any('hilton'));     # 667
ok($lcma->is_member_any('icon' ));      # 668
ok($lcma->is_member_any('jerky'));      # 669
ok(! $lcma->is_member_any('zebra'));    # 670

eval { $lcma->is_member_any('jerky', 'zebra') };
...
eval { $memb_hash_ref = $lcma->are_members_any() };
...
my $lcma_dj   = List::Compare->new(\@a0, \@a1, \@a2, \@a3, \@a4, \@a8);
...
eval { $disj = $lcma_dj->is_LdisjointR(2) };
ok(ok_capture_error($@));               # 689

########## BELOW:  Tests for '-u' option ##########

my $lcmau   = List::Compare->new('-u', '-a', \@a0, \@a1, \@a2, \@a3, \@a4);
...
ok($lcmau->is_member_any('abel'));      # 1146
ok($lcmau->is_member_any('baker'));     # 1147
ok($lcmau->is_member_any('camera'));    # 1148
ok($lcmau->is_member_any('delta'));     # 1149
ok($lcmau->is_member_any('edward'));    # 1150
ok($lcmau->is_member_any('fargo'));     # 1151
ok($lcmau->is_member_any('golfer'));    # 1152
ok($lcmau->is_member_any('hilton'));    # 1153
ok($lcmau->is_member_any('icon' ));     # 1154
ok($lcmau->is_member_any('jerky'));     # 1155
ok(! $lcmau->is_member_any('zebra'));   # 1156
...
my $lcmaun   = List::Compare->new('--unsorted', '--accelerated', 
                   \@a0, \@a1, \@a2, \@a3, \@a4);
...
eval { $lca_bad = List::Compare->new('-a', \@a0, \@a1, \%h5) };
ok(ok_capture_error($@));               # 1170

eval { $lca_bad = List::Compare->new('-a', \%h5, \@a0, \@a1) };
ok(ok_capture_error($@));               # 1171

my $scalar = 'test';
eval { $lca_bad = List::Compare->new('-a', \$scalar, \@a0, \@a1) };
ok(ok_capture_error($@));               # 1172
