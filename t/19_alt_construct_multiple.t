# 19_alt_construct_multiple.t # as of 8/7/2004
...
ok($lcm->is_member_any('abel'));        # 652
ok($lcm->is_member_any('baker'));       # 653
ok($lcm->is_member_any('camera'));      # 654
ok($lcm->is_member_any('delta'));       # 655
ok($lcm->is_member_any('edward'));      # 656
ok($lcm->is_member_any('fargo'));       # 657
ok($lcm->is_member_any('golfer'));      # 658
ok($lcm->is_member_any('hilton'));      # 659
ok($lcm->is_member_any('icon' ));       # 660
ok($lcm->is_member_any('jerky'));       # 661
ok(! $lcm->is_member_any('zebra'));     # 662
...
my $lcm_dj   = List::Compare->new( { lists => [\@a0, \@a1, \@a2, \@a3, \@a4, \@a8] } );
...
my $lcmu   = List::Compare->new( { unsorted => 1, lists => [\@a0, \@a1, \@a2, \@a3, \@a4] } );
...
ok($lcmu->is_member_any('abel'));       # 1135
ok($lcmu->is_member_any('baker'));      # 1136
ok($lcmu->is_member_any('camera'));     # 1137
ok($lcmu->is_member_any('delta'));      # 1138
ok($lcmu->is_member_any('edward'));     # 1139
ok($lcmu->is_member_any('fargo'));      # 1140
ok($lcmu->is_member_any('golfer'));     # 1141
ok($lcmu->is_member_any('hilton'));     # 1142
ok($lcmu->is_member_any('icon' ));      # 1143
ok($lcmu->is_member_any('jerky'));      # 1144
ok(! $lcmu->is_member_any('zebra'));    # 1145
...
my $lcmu_dj   = List::Compare->new( { lists => [\@a0, \@a1, \@a2, \@a3, \@a4, \@a8] } );
...
my $lcmun   = List::Compare->new( { unsorted => 1, lists => [\@a0, \@a1, \@a2, \@a3, \@a4] } );
...
eval { $lcm_bad = List::Compare->new( { accelerated => 1, lists => [\@a0, \@a1, \@a2, \@a3, \%h5] } ) };
ok(ok_capture_error($@));               # 1163

eval { $lcm_bad = List::Compare->new( { accelerated => 1, lists => [\%h5, \@a0, \@a1, \@a2, \@a3] } ) };
ok(ok_capture_error($@));               # 1164
